...
import { Users, Plus, Edit2, Trash2, History, Search, X, RefreshCw } from 'lucide-react';
...
export default function Customers() {
  ...
  // FIX: added try/catch and loading state — original had no error handling
  ...
  const addrSection = (title, type) => (
    <div className="mt-4">
      ...
      <div className="flex flex-col gap-2">
        <input
          className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:bg-white transition"
          placeholder="Street"
              ...
              <tbody>
                ...
                {filtered.map(c => (
                  <tr key={c._id} className="border-b border-gray-50 hover:bg-gray-50 transition last:border-0">
                    ...
                    <td className="px-5 py-3.5 text-sm text-gray-600">
                      {c.contactPerson || <span className="text-gray-300">—</span>}
                    </td>
                    <td className="px-5 py-3.5 text-sm font-medium text-red-600">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              ...
              <button onClick={() => setShowLedger(false)} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 transition">
                <X size={18}/>
              </button>
            </div>
              ...
              {ledgerLoading ? (
                ...
                <table className="w-full min-w-[500px]">
                  ...
                  <tbody>
                    {ledgerData.length === 0 && (
                      <tr>
                        <td colSpan={6} className="text-center py-10 text-gray-400 text-sm">No transactions found</td>
                      </tr>
                    )}
                    {ledgerData.map(o => (
