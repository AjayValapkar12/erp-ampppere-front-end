...
export default function Purchasing() {
  ...
  const loadVendors = async () => { try { const res=await api.get('/vendors'); setVendors(res.data.data); } catch(e){} finally{setLoading(false);} };
  const loadOrders = async () => { try { const res=await api.get('/purchases'); setOrders(res.data.data); } catch(e){} };

  const openVendorCreate = () => { setEditVendor(null); setVendorForm(emptyVendor); setError(''); setShowVendorModal(true); };
  const openVendorEdit = (v) => { setEditVendor(v); setVendorForm({ name:v.name||'', email:v.email||'', phone:v.phone||'', contactPerson:v.contactPerson||'', gstNumber:v.gstNumber||'', address:v.address||emptyVendor.address }); setError(''); setShowVendorModal(true); };
  const openOrderCreate = () => { setEditOrder(null); setOrderForm({ vendor:'', orderDate:formatDateInput(new Date()), expectedDate:'', notes:'', items:[{...EMPTY_ITEM}] }); setError(''); setShowOrderModal(true); };
  const openOrderEdit = (o) => { setEditOrder(o); setOrderForm({ vendor:o.vendor?._id||o.vendor, orderDate:formatDateInput(o.orderDate), expectedDate:formatDateInput(o.expectedDate), notes:o.notes||'', items:o.items.map(i=>({...i})) }); setError(''); setShowOrderModal(true); };
  const openView = async (o) => { const res=await api.get(`/purchases/${o._id}`); setViewOrder(res.data.data); setShowViewModal(true); };
  const openPay = (o) => { setPayOrder(o); setPayForm({ amount:'', paymentMethod:'bank_transfer', transactionId:'', notes:'' }); setError(''); setShowPayModal(true); };

  const updateItem = (idx, field, val) => { const items=[...orderForm.items]; items[idx]={...items[idx],[field]:val}; setOrderForm(f=>({...f,items})); };

  const calcTotals = () => {
    let sub=0,gst=0;
    orderForm.items.forEach(i=>{ const a=(parseFloat(i.quantity)||0)*(parseFloat(i.rate)||0); sub+=a; gst+=a*(parseFloat(i.gstRate)||0)/100; });
  ...
  const handleSaveVendor = async () => {
    ...
    try {
      if (editVendor) { const res=await api.put(`/vendors/${editVendor._id}`,vendorForm); setVendors(v=>v.map(x=>x._id===editVendor._id?res.data.data:x)); }
      else { const res=await api.post('/vendors',vendorForm); setVendors(v=>[res.data.data,...v]); }
  ...
  const handleDeleteVendor = async (v) => {
    ...
    try { await api.delete(`/vendors/${v._id}`); setVendors(x=>x.filter(vx=>vx._id!==v._id)); } catch(e){ alert('Error deleting'); }
  };
  ...
  const handleSaveOrder = async () => {
    ...
    setSaving(true); setError('');
    try {
      if (editOrder) { const res=await api.put(`/purchases/${editOrder._id}`,orderForm); setOrders(o=>o.map(x=>x._id===editOrder._id?res.data.data:x)); }
      else { const res=await api.post('/purchases',orderForm); setOrders(o=>[res.data.data,...o]); }
  ...
  const handleDeleteOrder = async (o) => {
    ...
    try { await api.delete(`/purchases/${o._id}`); setOrders(x=>x.filter(ox=>ox._id!==o._id)); } catch(e){ alert('Error deleting'); }
  };

  const handlePayment = async () => {
    if (!payForm.amount||parseFloat(payForm.amount)<=0) return setError('Enter valid amount');
    setSaving(true); setError('');
    try {
      const res=await api.post(`/purchases/${payOrder._id}/payment`,payForm);
      ...
      {tab==='vendors' && (
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
            <span className="text-sm font-semibold text-gray-900">Vendors <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full ml-1">{vendors.length}</span></span>
            <button onClick={openVendorCreate} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-gray-900 text-white rounded-lg hover:bg-gray-700 transition"><Plus size={13}/>Add Vendor</button>
          </div>
      ...
      {tab==='orders' && (
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
            ...
            <button onClick={openOrderCreate} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-gray-900 text-white rounded-lg hover:bg-gray-700 transition"><Plus size={13}/>Create Order</button>
          </div>
          <div className="overflow-x-auto">
                  ...
                  <tr key={o._id} className="border-b border-gray-50 hover:bg-gray-50 transition last:border-0">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={()=>openPay(o)} disabled={o.paymentStatus==='paid'} className="p-1.5 rounded-lg border border-gray-100 text-gray-500 hover:bg-green-50 hover:text-green-600 transition disabled:opacity-30"><CreditCard size={13}/></button>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-xl">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[['Vendor Name *','name',''],['Phone','phone',''],['Email','email',''],['Contact Person','contactPerson',''],['GST Number','gstNumber','']].map(([label,field,ph])=>(
                  field==='gstNumber'
                    ? <div key={field} className="sm:col-span-2"><label className={labelCls}>{label}</label><input className={inputCls} value={vendorForm[field]} onChange={e=>setVendorForm(f=>({...f,[field]:e.target.value}))} placeholder={ph}/></div>
                    : <div key={field}><label className={labelCls}>{label}</label><input className={inputCls} value={vendorForm[field]} onChange={e=>setVendorForm(f=>({...f,[field]:e.target.value}))} placeholder={ph}/></div>
                ))}
              </div>
              <div>
                      ...
                      {orderForm.items.map((item,idx)=>{
                        ...
                        return (
                          <tr key={idx} className="border-b border-gray-50 last:border-0">
                            ...
                            <td className="px-2 py-2">
                              {orderForm.items.length>1&&<button onClick={()=>setOrderForm(f=>({...f,items:f.items.filter((_,i)=>i!==idx)}))} className="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition"><X size={14}/></button>}
                            </td>
                          </tr>
                        );
                      })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div><label className={labelCls}>Amount *</label><input type="number" className={inputCls} value={payForm.amount} onChange={e=>setPayForm(f=>({...f,amount:e.target.value}))} max={payOrder.outstandingAmount}/></div>
              ...
              <div><label className={labelCls}>Transaction ID / Reference</label><input className={inputCls} value={payForm.transactionId} onChange={e=>setPayForm(f=>({...f,transactionId:e.target.value}))} placeholder="Optional"/></div>
            </div>
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-100">
